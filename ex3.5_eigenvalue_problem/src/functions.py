...
# Allows to call model(X_f, N) where N is the quantum number of the eigenstate, 
# and it will be concatenated to the input
class PINNModel(nn.Module):
    def __init__(self, cfg: ModelConfig, input_dim: int):
        super().__init__()
        input_dim = input_dim + 1  # +1 for quantum number

        layers = [nn.Linear(input_dim, cfg.neurons_per_layer), nn.Tanh()]
    ...
    def forward(self, X_f: torch.Tensor, N: torch.Tensor) -> torch.Tensor:
        # N shape: (batch,) -> (batch, 1), cast to float for concatenation
        n_col = N.float().unsqueeze(1)
        x = torch.cat([X_f, n_col], dim=1)
        return self.net(x)

def build_model(cfg: ModelConfig, input_dim: int, device, load_path=None) -> PINNModel:
    model = PINNModel(cfg, input_dim)
...
def make_collocation_points(cfg: DomainConfig, device):
    ...
    # Repeat for each eigenstate
    X_f = X_f.repeat(cfg.N_eigenstates, 1)  # (num_eigenstates * N_points, input_dim)
    V   = V.repeat(cfg.N_eigenstates, 1)     # (num_eigenstates * N_points, 1)

    # Quantum number column
    N = torch.arange(cfg.N_eigenstates, dtype=torch.long, device=device)
    N = N.repeat_interleave(cfg.N_points)      # (num_eigenstates * N_points,)
...
def compute_loss(model, mu, h, X_f, N, potential, domain: DomainConfig):
    laplacian = compute_laplacian(h, X_f, domain.input_dim)

    # mu is shape (num_eigenstates,), expand to match h shape (num_eigenstates * N_points, 1)
    mu_expanded = mu[N].unsqueeze(1)  # (num_eigenstates * N_points, 1)

    L_pde = (-0.5 * laplacian + (potential + domain.Na * h.pow(2) - mu_expanded) * h).pow(2).mean()
    ...
    # BC points repeated for each eigenstate
    N_bc = torch.arange(domain.N_eigenstates, dtype=torch.long, device=h.device)
    N_bc = N_bc.repeat_interleave(domain.BC_grid.shape[0])
    BC_grid_t = BC_grid_t.repeat(domain.N_eigenstates, 1)
    BC_h_t = BC_h_t.repeat(domain.N_eigenstates, 1)

    L_bc = (model(BC_grid_t, N_bc) - BC_h_t).pow(2).mean()

    volume = np.prod(np.array(domain.x_r) - np.array(domain.x_l))
    h_per_state = h.view(domain.N_eigenstates, -1, 1)
    L_norm = (h_per_state.pow(2).mean(dim=1) * volume - 1.0).pow(2).mean()

    # Orthogonality condition
    L_ortho = 0
    for i in range(domain.N_eigenstates):
        for j in range(i + 1, domain.N_eigenstates):
            hi = h_per_state[i]
            hj = h_per_state[j]
            L_ortho += ((hi * hj).mean() * volume).pow(2)

    return L_pde + domain.BC_w * L_bc + 10 * L_norm + L_ortho
...
class GPESolver:
    def __init__(self, domain: DomainConfig, model_cfg: ModelConfig,
                 train_cfg: TrainConfig, load_model=None):
        self.domain = domain
        ...
        self.mu = nn.Parameter(torch.tensor([0.0] * domain.N_eigenstates, device=self.device))
        self.X_f_base, self.potential, self.N_eig = make_collocation_points(domain, self.device)
    ...
    def _forward(self):
        ...
        h = self.model(X_f, self.N_eig)
        return h, X_f

    def _loss(self, h, X_f):
        return compute_loss(self.model, self.mu, h, X_f, self.N_eig, self.potential, self.domain)

    def _update_best_loss(self):
        if self.loss_history[-1] < self.best_loss:
            self.best_loss = self.loss_history[-1]
            self.best_mu = self.mu.tolist()
    ...
    def _train_adam(self, max_epochs, offset=0):
        ...
        for epoch in range(offset, max_epochs + offset + 1):
            ...
            if epoch % cfg.print_every == 0:
                mu_str = ", ".join(f"{v:.4f}" for v in self.mu.tolist())
                print(f"[Adam]   {epoch:6d} | loss {loss.item():.6e} | μ [{mu_str}]")
    ...
    def _train_lbfgs(self, max_epochs, offset=0):
        ...
        for epoch in range(offset, max_epochs + offset + 1):
            ...
            if epoch % cfg.print_every == 0:
                mu_str = ", ".join(f"{v:.4f}" for v in self.mu.tolist())
                print(f"[L-BFGS] {epoch:6d} | loss {current_loss[0]:.6e} | μ [{mu_str}]")
