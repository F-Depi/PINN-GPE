...
class DomainConfig:
    ...
    N_points: int = 10_000
    V_ext: Callable = field(default=lambda x: 0.0)
    Na: float = 1.0
    B: float = 0.0

    periodic_BC: tuple[np.ndarray, np.ndarray] = field(default_factory=lambda: (np.array([]), np.array([])))
    BC: np.ndarray = field(default_factory=lambda: np.array([]))
...
def build_model(cfg: ModelConfig, input_dim: int, device, load_path=None) -> nn.Sequential:
    ...
    layers.append(nn.Linear(cfg.neurons_per_layer, 2))
...
def compute_laplacian(h, X_f):
    ...
    u_yy = torch.autograd.grad(
        u_y, X_f, grad_outputs=torch.ones_like(u_y), create_graph=True
        )[0][:, 1:2]

    laplacian = u_xx + u_yy
    return u_x, u_y, laplacian


def compute_loss(model, mu, h, X_f, potential, domain: DomainConfig):
    
    u = h[:, 0:1]
    v = h[:, 1:2]

    u_x, u_y, lap_u = compute_laplacian(u, X_f)
    v_x, v_y, lap_v = compute_laplacian(v, X_f)
    ...
    L_pde = (
        (-0.5 * lap_u + domain.B * (x * v_y - y * v_x) + (potential + domain.Na * h2 - mu) * u)
        .pow(2)
        .mean()
    )

    L_pde += (
        (-0.5 * lap_v - domain.B * (x * u_y - y * u_x) + (domain.Na * h2 - mu) * v)
        .pow(2)
        .mean()
    )

    BC = torch.tensor(domain.BC, dtype=torch.float32, device=h.device)
    BC_h = torch.tensor(domain.BC_h, dtype=torch.float32, device=h.device)

    L_bc = (model(BC) - BC_h).pow(2).mean()

    return L_pde + domain.BC_w * L_bc
...
class GPESolver:
    def __init__(self, domain: DomainConfig, model_cfg: ModelConfig,
        ...
        self.mu = torch.nn.Parameter(torch.tensor(domain.Na, device=self.device))
    ...
    def _loss(self, h, X_f):
        return compute_loss(self.model, self.mu, h, X_f, self.potential, self.domain)
